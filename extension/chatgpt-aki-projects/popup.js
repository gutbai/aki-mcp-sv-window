...
function normalizeProjectPath(value) { return value.trim().replace(/^"(.*)"$/, '$1').trim(); }
function folderName(folderPath) { return normalizeProjectPath(folderPath).split(/[\\/]/).filter(Boolean).pop() || ''; }
...
pathInput.addEventListener('change', () => { const normalized = normalizeProjectPath(pathInput.value); pathInput.value = normalized; if (!nameInput.value.trim() && normalized) nameInput.value = folderName(normalized); });
form.addEventListener('submit', async (event) => { event.preventDefault(); const name = nameInput.value.trim(); const projectPath = normalizeProjectPath(pathInput.value); pathInput.value = projectPath; if (!name || !projectPath) { setMessage('Hãy nhập tên project và dán hoặc chọn folder project.'); return; } if (editingId) { projects = projects.map((project) => project.id === editingId ? { ...project, name, path: projectPath } : project); setMessage(`Đã cập nhật ${name}.`); } else { const project = { id: crypto.randomUUID(), name, path: projectPath }; projects.push(project); if (!defaultProjectId) defaultProjectId = project.id; setMessage(`Đã thêm ${name}.`); } await persist(); resetForm(); render(); });
