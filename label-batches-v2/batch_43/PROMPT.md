# Lô 43/43 — định vị vật thể theo mẫu tham chiếu
...
## Việc cần làm
Thư mục `images/` có 38 bài, đặt tên `batch_43_img001` … `batch_43_img038`. Mỗi bài gồm 4 file ảnh:
...
(không ghi trong repo). Tải về, giải nén ra thư mục `images/` gồm toàn bộ `.jpg`/`.png`, rồi mở
xem trực tiếp. Một lần tải là đủ cho cả lô.

Nếu không được đưa đường dẫn, hãy **hỏi người giao việc**, đừng tự đi tìm nguồn khác.
...
- Làm hết 38 bài, không bỏ bài nào — nhưng "làm" gồm cả việc ĐÁNH DẤU BỎ QUA (xem dưới).
...
```
git add batch_43/RESULT.json
git commit -m "result: batch_43"
